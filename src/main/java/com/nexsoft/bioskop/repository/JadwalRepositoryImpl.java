...
public class JadwalRepositoryImpl implements JadwalRepository {
    ...
    public List<Jadwal> findAll(int limit, int offset) {
        ...
        try {
            jadwalList = this.jdbcTemplate.query("SELECT * FROM jadwal LIMIT ? OFFSET ?",
                    preparedStatement -> {
    ...
    @Override
    public List<Jadwal> findByTanggal(LocalDate tanggal, int limit, int offset) {
        List<Jadwal> jadwalList;

        try {
            jadwalList = this.jdbcTemplate.query("SELECT * FROM jadwal WHERE tanggal = ? LIMIT ? OFFSET ?",
                    preparedStatement ->
                    {
    ...
    @Override
    public List<Jadwal> findByRuang(String ruang, int limit, int offset) {
        List<Jadwal> jadwalList;

        try {
            jadwalList = this.jdbcTemplate.query("SELECT j.id, j.tanggal, j.id_ruang, j.id_film, j.jam FROM jadwal j, ruang r WHERE j.id_ruang = r.id AND r.nama LIKE ? LIMIT ? OFFSET ?;",
                    preparedStatement -> {
                        preparedStatement.setString(1, "%" + ruang + "%");
                        preparedStatement.setInt(2, limit);
                        preparedStatement.setInt(3, offset);
                    },
                    (rs, rowNum) ->
    ...
    @Override
    public List<Jadwal> findByFilm(String judul, int limit, int offset) {
        List<Jadwal> jadwalList;

        try {
            jadwalList = this.jdbcTemplate.query("SELECT j.id, j.tanggal, j.id_ruang, j.id_film, j.jam FROM jadwal j, film f WHERE j.id_film = f.id AND f.judul LIKE ? LIMIT ? OFFSET ?;",
                    preparedStatement -> {
    ...
    public List<Jadwal> findByFilmForCustomer(String idFilm) {
        ...
        try {
            jadwalList = this.jdbcTemplate.query("SELECT j.id, j.tanggal, j.id_ruang, j.id_film, j.jam, r.jenis \n" +
                            "FROM ((jadwal j \n" +
                            "       INNER JOIN film f ON j.id_film = f.id)\n" +
                            "       INNER JOIN ruang r ON j.id_ruang = r.id)\n" +
                            "       WHERE f.id = ? AND j.tanggal >= ? ORDER BY j.tanggal, r.jenis;",
                    preparedStatement -> {
